...
class TestAsyncComment(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_create_comment(self):
        with patch('src.repository.comments.Comment', return_value=MagicMock()) as mock_comment:
            result = await create_comment(self.comment, self.photo_id, self.user_id, self.session)
    ...
    async def test_get_user_comments_for_photo(self):
        comments = [CommentSchema(opinion="Test comment one"),
    ...
    async def test_edit_comment(self):
        ...
        mocked_comment = MagicMock()
        mocked_comment.scalars_or_none.return_value = CommentSchema(opinion="Test comment")
        self.session.execute.return_value = mocked_comment
...
async def test_create_comment():
    ...
    with patch('src.repository.comments.Comment', return_value=MagicMock()) as mock_comment:
        ...
        mock_comment.assert_called_once_with(opinion=body.opinion, photo_id=photo_id, user_id=user_id)
